...
async function loadMinisterData() {
    try {
        ...
        ministerData.ministers = sortMinistersChronologically(ministerData.ministers);
...
function getMinisterInfo(name) {
    return ministerOrder.find(m => m.name === name) || { start: 1960, end: 1965, era: 'founding', image: '' };
}

// Format minister name (shorter)
function shortName(name) {
    return name.replace('Dr ', '').replace(' Tsu Tau', '').replace(' Keng Yam', '');
}
...
function renderMetricsSummary() {
    ...
    const totalSentences = ministers.reduce((sum, m) => sum + m.total_sentences, 0);
    const totalSpeeches = ministers.reduce((sum, m) => sum + m.num_speeches, 0);
    const avgReadability = ministers.reduce((sum, m) => sum + m.readability_score, 0) / ministers.length;
    const longestServing = ministers.reduce((a, b) => a.num_speeches > b.num_speeches ? a : b);
    ...
    container.innerHTML = `
        <div class="metrics-grid">
            ...
            <div class="metric-card">
                <div class="metric-value">${(totalSentences / 1000).toFixed(0)}k</div>
...
function renderTimelineChart() {
    ...
    const layout = {
        ...
        annotations: [
            { x: 1962, y: 10.5, text: '<b>Founding Era</b>', showarrow: false, font: { size: 10, color: eraColors.founding } },
            { x: 1977, y: 10.5, text: '<b>Growth Era</b>', showarrow: false, font: { size: 10, color: eraColors.growth } },
            { x: 1995, y: 10.5, text: '<b>Maturity Era</b>', showarrow: false, font: { size: 10, color: eraColors.maturity } },
            { x: 2015, y: 10.5, text: '<b>Modern Era</b>', showarrow: false, font: { size: 10, color: eraColors.modern } }
        ]
    };
...
function renderOutputChart() {
    const ministers = ministerData.ministers;
    const avgSentences = ministers.reduce((sum, m) => sum + m.total_sentences, 0) / ministers.length;
    ...
    const trace = {
        ...
        marker: {
            size: 14,
            color: ministers.map(m => m.total_sentences >= avgSentences ? colors.success : colors.accent),
            line: { color: 'white', width: 2 }
        },
        text: ministers.map(m => m.total_sentences.toLocaleString()),
        textposition: 'middle right',
        textfont: { size: 10 },
        hovertemplate: '%{y}<br>%{x:,} sentences<br>%{customdata} speeches<extra></extra>',
    ...
    const shapes = ministers.map(m => ({
        ...
        y0: shortName(m.name),
        y1: shortName(m.name),
        line: { color: m.total_sentences >= avgSentences ? colors.success : colors.accent, width: 3 }
    }));
    ...
    Plotly.newPlot('outputChart', [trace], layout, { responsive: true, displayModeBar: false });
}
...
function renderStyleChart() {
    ...
    const trace = {
        ...
        textfont: { size: 9 },
        hovertemplate: '<b>%{customdata}</b><br>Sentence Length: %{x:.1f} words<br>Readability: %{y:.1f}<extra></extra>',
        customdata: ministers.map(m => m.name)
    };
    
    // Add quadrant lines at averages
    const avgLen = ministers.reduce((sum, m) => sum + m.avg_sentence_length, 0) / ministers.length;
    const avgRead = ministers.reduce((sum, m) => sum + m.readability_score, 0) / ministers.length;
    ...
    const layout = {
        ...
        shapes: [
            ...
            { type: 'line', x0: avgLen, x1: avgLen, y0: 50, y1: 70, line: { color: colors.grid, width: 1, dash: 'dot' } },
            // Horizontal line at avg readability
            { type: 'line', x0: 15, x1: 25, y0: avgRead, y1: avgRead, line: { color: colors.grid, width: 1, dash: 'dot' } }
        ],
        height: 400,
        showlegend: false,
        paper_bgcolor: 'transparent',
        ...
        annotations: [
            { x: 16, y: 68, text: 'Clear & Concise', showarrow: false, font: { size: 10, color: colors.success } },
            { x: 24, y: 52, text: 'Complex', showarrow: false, font: { size: 10, color: colors.accent } }
        ]
    };
    
    Plotly.newPlot('styleChart', [trace], layout, { responsive: true, displayModeBar: false });
}
...
function renderTopicsHeatmap() {
    ...
    ministers.forEach(minister => {
        ...
        Object.entries(topics).forEach(([topic, value]) => {
            if (topic !== 'General') {  // Exclude General as it's too dominant
    ...
    Plotly.newPlot('topicsHeatmap', [trace], layout, { responsive: true, displayModeBar: false });
}
...
function renderMinisterProfiles() {
    ...
    // Calculate rankings
    const readabilityRanked = [...ministers].sort((a, b) => b.readability_score - a.readability_score);
    const sentenceRanked = [...ministers].sort((a, b) => a.avg_sentence_length - b.avg_sentence_length);
    ...
    container.innerHTML = `
        ...
        <div class="minister-cards">
            ${ministers.map(minister => {
                const info = getMinisterInfo(minister.name);
                const readRank = readabilityRanked.findIndex(m => m.name === minister.name) + 1;
                const sentRank = sentenceRanked.findIndex(m => m.name === minister.name) + 1;
                
                // Get top topics for this minister
                const topics = ministerData.minister_topics[minister.name] || {};
                ...
                return `
                    <div class="minister-card" style="border-left: 4px solid ${eraColors[info.era]};">
                        <div class="minister-card-header">
                            <div class="minister-portrait">
                                <img src="${info.image}" alt="${minister.name}" loading="lazy" onerror="this.style.display='none'">
                            </div>
                            <div class="minister-header-text">
                                <h4>${minister.name}</h4>
                                <span class="minister-tenure">${minister.years_served}</span>
                            </div>
                        </div>
                        
                        <div class="minister-stats-grid">
                            <div class="minister-stat">
                                <span class="stat-value">${minister.num_speeches}</span>
                            ...
                            <div class="minister-stat">
                                <span class="stat-value">${minister.readability_score.toFixed(0)}</span>
                                <span class="stat-label">Readability</span>
                                <span class="stat-rank ${readRank <= 3 ? 'top' : ''}">#${readRank}</span>
                            </div>
                            <div class="minister-stat">
                                <span class="stat-value">${minister.avg_sentence_length.toFixed(1)}</span>
                                <span class="stat-label">Words/Sentence</span>
                                <span class="stat-rank ${sentRank <= 3 ? 'top' : ''}">#${sentRank}</span>
                            </div>
                        </div>
